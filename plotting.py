    ...
    def plot_spectrum_dataset(self, dataset: Dataset):
        ...
        for data in dataset.items:
            ...
            if spec_spl is None:
                continue
            if data.derived.get('psf_2d_spl') is not None :
                self.plot_spectrum(data, spec_spl, title=f"Final Spectrum after 2D - {data.file_id}")
            else :
